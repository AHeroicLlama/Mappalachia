...
## MapIconProcessor
Now in `Mappalachia\MapIconProcessor` launch the `MapIconProcessor` solution. Compile and run it.<br/>
Much like the database preprocessor, this is a very small program whose job is to copy only the necessary map marker icon images, rename them suitably, and verify that all icons requested by the database are accounted for.<br/>
