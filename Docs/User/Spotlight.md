...
## Updating Spotlight
If you already have the latest version of spotlight installed, you may download only newly updated parts of it for the current patch by downloading all `patch_spotlight.7z.XXX` files from the [latest release](https://github.com/AHeroicLlama/Mappalachia/releases/latest), and similarly unzipping them into the `img\` folder, merging with the `spotlight\` folder you already have there.<br/>
